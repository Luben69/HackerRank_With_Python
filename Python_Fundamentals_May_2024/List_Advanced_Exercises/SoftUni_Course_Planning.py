...
def insert_lessons(schedule, lesson_title, index):
    if lesson_title not in schedule:
        schedule.insert(index, lesson_title)


def add_lessons(schedule, lesson_title):
    if lesson_title not in schedule:
        schedule.append(lesson_title)
...
def process_commands(schedule, commands):
    for command in commands:
        ...
        if action == 'Add':
            ...
            add_lessons(schedule, lesson_title)

        elif action == 'Insert':
            lesson_title = parts[1]
            index = int(parts[2])
            insert_lessons(schedule, lesson_title, index)
            ...
            lesson_title = parts[1]
            other_lesson_title = parts[2]
            swap_lessons(schedule, lesson_title, other_lesson_title)
